...
class App(tkinter.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.search_bar = CTkEntry(self, width=50)
        self.search_bar.grid(row=0, column=0, pady=10, padx=10, sticky="we")
        self.search_bar.focus()

        self.search_bar_button = CTkButton(master=self, width=8, text="Search", command=self.search)
        self.search_bar_button.grid(row=0, column=1, pady=10, padx=10)

        self.search_bar_clear = CTkButton(master=self, width=8, text="Clear", command=self.clear)
        ...
        self.listbox_button_frame = CTkFrame(master=self)
        ...
        self.save_marker_button = CTkButton(master=self.listbox_button_frame, width=20, text="save current marker",
                                                 command=self.save_marker)
        self.save_marker_button.grid(row=0, column=0, pady=10, padx=10)

        self.clear_marker_button = CTkButton(master=self.listbox_button_frame, width=20, text="clear marker list",
                                                  command=self.clear_marker_list)
        self.clear_marker_button.grid(row=1, column=0, pady=10, padx=10)

        self.connect_marker_button = CTkButton(master=self.listbox_button_frame, width=20, text="connect marker with path",
